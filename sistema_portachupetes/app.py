...
from crud.pedidos import crear_pedido, obtener_materiales_utilizados, cancelar_pedido, modificar_pedido, terminar_pedido, listar_todos_pedidos,obtener_pedido
import pandas as pd
import plotly.express as px
import plotly.graph_objects as go
from database.engine import engine
from sqlalchemy.orm import Session
from database.models import MaterialPedido
...
### PEDIDOS ###
    ...
    tabs_pedido = st.tabs(['Generar Pedido :smile:', 'Cancelar pedido :angry:', 'Terminar pedido ✅', 'Actualizar pedido :zipper_mouth_face:', 'Listar pedidos :alien:', 'Proximamente ... :dizzy_face:'])
        ...
        with st.form('form_terminar_pedido', border=False):
            if not df_pedidos_filtrado.empty:  # type: ignore
                pedido_a_terminar = st.selectbox(
                    ...
                    sorted(df_pedidos_filtrado['ID'].unique()) #type:ignore
                )
...
### METRICAS ###
    ...
    with tabs_metricas[0]:
        ...
        with col3:
            st.metric('Cancelados', value=dif_cancelados, delta=pct_cancelados, delta_color= 'inverse' if dif_cancelados > 0 else 'normal')

        with col4:
            st.metric('Terminados', value=dif_terminados, delta=pct_terminados, delta_color= 'inverse' if dif_terminados > 0 else 'normal')
