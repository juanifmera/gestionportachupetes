...
def crear_pedido(cliente: str, materiales_portachupete: dict, estado="En proceso", fecha_pedido=datetime.today(), telefono=""):
    try:
        ...
        if letras_procesadas > 5:
            ...
            mensaje = f"✅ Pedido generado con éxito para {cliente.capitalize()} (ID: {nuevo_pedido.id}) - Costo Total: ${int(costo_total):,}" + f'⚠️ Se aplicó un cargo adicional de {extras} letra(s) extra × ${cargo_extra_letra}.'
